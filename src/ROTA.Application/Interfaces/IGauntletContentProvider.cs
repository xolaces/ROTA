...
public interface IGauntletContentProvider
{
    ...
    // kind-aware band lookup. Neck = the standard bands; Ring = the ring set when authored,
    ...
    // the full kind-aware band list (same Ring fallback rule as GetBandForRank), ordered by
    ...
    // a Gauntlet raid stage by its raid-definition id (the combat kill hook's reverse
